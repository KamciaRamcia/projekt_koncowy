...
from features.featrue_fishcard_list import NewListElement, listOfFiles
...
class FishCardListPage:
    ...
    def generateFishcardList(self):  # ZMIEN NAZW NA ADD_FISHCARD
        ...
        Label(self.root, text="Lista Twoich fiszek", bg='SkyBlue3', anchor='center',
              font=(FishCardListPage.your_font, 25, 'bold')).grid(padx=5, pady=5, row=0, column=1)

        NewListElement.button_image = PhotoImage(file="../images/button.png")

        for filename in listOfFiles():
            NewListElement(filename, self.root).create_elements()

        Button(self.root, text='Wróć do menu głównego', borderwidth=1,
               image=NewListElement.button_image,
               compound='center', width=191, font=(FishCardListPage.your_font, 18, 'bold'), height=25,
               command=lambda: self.back_to_menu_actions()).grid(row=2, column=1, pady=10)
