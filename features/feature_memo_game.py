...
class ButtonWithWord:
    ...
    def click_action(self, click_button):
        if click_button.cget(
                'bg') != ButtonWithWord.green_colour:  # spr czy przycisk nie zostal juz poprzednio poprawnie dopasowany
    ...
    def check_pair(self, click_button):
        if (ButtonWithWord.current_pair[0] in self.fishcard_dictionary) and self.fishcard_dictionary[
            ButtonWithWord.current_pair[0]] == ButtonWithWord.current_pair[1]:  # gdy pierwsze wybrane slowo to klucz
        ...
        elif (ButtonWithWord.current_pair[1] in self.fishcard_dictionary) and self.fishcard_dictionary[
            ButtonWithWord.current_pair[1]] == ButtonWithWord.current_pair[0]:  # gdy drugie wybrane slowo to klucz
            ButtonWithWord.green_counter = ButtonWithWord.green_counter + 1
            self.correct_pair(ButtonWithWord.current_pair[0], ButtonWithWord.current_pair[1])
...
def shuffle_dictionary(fishcard_dictionary: dict):
